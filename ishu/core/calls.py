        ...
        if media_path:
            try:
                stream = types.MediaStream(
                    ...
                    ffmpeg_parameters=(f"-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -http_persistent 0 -probesize 5M -analyzeduration 5000000" + (f" -ss {seek_time}" if seek_time > 1 else "")) if str(media_path).startswith("http") else (f"-ss {seek_time}" if seek_time > 1 else None),
                )
                await client.play(
                    chat_id=chat_id,
                    stream=stream,
